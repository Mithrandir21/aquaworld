...
public class CanvasManagment
{
			...
			WorkareaCanvas[] canvases)
	{
		for ( int i = 0; i < canvases.length; i++ )
		{
			if ( canvases[i] != null )
			{
				if ( canvases[i].getTankName().equals(canvasName) )
				{
					return canvases[i];
				}
			}
...
//	 *         found.
//	 */
//	public static WorkareaCanvas findCanvas(WidgetTransferable obj,
//			WorkareaCanvas[] canvases)
//	{
...
//	}
	 ...
	 */
	public static WidgetTransferable findWidgetObject(WidgetTransferable obj,
			WorkareaCanvas canvas)
	{
		...
		{
			WidgetTransferable temp = (WidgetTransferable) iter.next();

			if ( temp.getWidgetSerial() == obj.getWidgetSerial() )
			{
				return temp;
			}
		}
	 ...
	 * @return The WidgetObject that contains the given object.
	 */
	public static WidgetTransferable findWidgetObjectByObjectName(String name,
			WorkareaCanvas canvas)
	{
		List<Widget> children = canvas.getMainLayer().getChildren();
		...
		for ( Iterator<Widget> iter = children.iterator(); iter.hasNext(); )
		{
			...
			if ( temp.getWidgetObjectName().equalsIgnoreCase(name) )
			{
				return temp;
			}
			...
			WorkareaCanvas[] canvases)
	{
		for ( int i = 0; i < canvases.length; i++ )
		{
			if ( canvases[i] != null )
			{
				if ( canvases[i].getTankName().equalsIgnoreCase(
						canvas.getTankName()) )
				{
					return true;
				}
			...
			WorkareaCanvas[] canvases)
	{
		for ( int i = 0; i < canvases.length; i++ )
		{
			if ( canvases[i] != null )
			{
				if ( canvases[i].getTankName().equalsIgnoreCase(canvasName) )
				{
					return true;
				}
			}
